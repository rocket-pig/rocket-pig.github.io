...
const genId = (n) => new Array(n).fill().map(() => charSet[Math.floor(Math.random() * charSet.length)]).join("");
...
const events = fromEntries(["close", "connect", "data", "error", "signal", "stream", "track"].map((k) => [
  k,
  k
]));
...
const algo = "AES-CBC";
const pack = (buff) => window.btoa(String.fromCharCode.apply(null, new Uint8Array(buff)));
const unpack = (packed) => {
  const str = window.atob(packed);
  return new Uint8Array(str.length).map((_, i) => str.charCodeAt(i)).buffer;
};
const genKey = async (secret, ns) => crypto.subtle.importKey("raw", await crypto.subtle.digest({name: "SHA-256"}, encodeBytes(`${secret}:${ns}`)), {name: algo}, false, ["encrypt", "decrypt"]);
const encrypt = async (keyP, plaintext) => {
  const iv = crypto.getRandomValues(new Uint8Array(16));
  return JSON.stringify({
    c: pack(await crypto.subtle.encrypt({name: algo, iv}, await keyP, encodeBytes(plaintext))),
    iv: [...iv]
  });
};
const decrypt = async (keyP, raw) => {
  const {c, iv} = JSON.parse(raw);
  return decodeBytes(await crypto.subtle.decrypt({name: algo, iv: new Uint8Array(iv)}, await keyP, unpack(c)));
};
export {initPeer as a, genId as b, events as c, entries as d, encodeBytes as e, fromEntries as f, genKey as g, encrypt as h, initGuard as i, decrypt as j, keys as k, libName as l, mkErr as m, noOp as n, decodeBytes as o, room as r, selfId as s, values as v};
export default null;
